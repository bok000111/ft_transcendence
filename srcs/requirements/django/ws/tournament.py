...
class TournamentManager:
    ...
    class Tournament:
        ...
        async def start_tournament(self):
            ...
            game1 = asyncio.create_task(self.room_manager.start_game(
                GameType.SUB_GAME, self.tournament_users[:2]))
            game2 = asyncio.create_task(self.room_manager.start_game(
                GameType.SUB_GAME, self.tournament_users[2:]))
            self.games.append(await game1)
            self.games.append(await game2)
            # games 둘 다 끝날 때까지 대기(game 나중에 끝나면 제거해주기)
            while (self.room_manager.check_status(self.games[0]) != "end" or self.room_manager.check_status(self.games[1]) != "end"):
                await asyncio.sleep(1)
            await asyncio.sleep(2)
            # final game
            final_users = []
            if self.tournament_users[0][0] == self.room_manager.get_game_instance(self.games[0]).get_winner():
            ...
            if self.tournament_users[2][0] == self.room_manager.get_game_instance(self.games[1]).get_winner():
            ...
            self.games.append(await self.room_manager.start_game(
