...
contract TournamentManager {
    ...
    function test_start(uint64 id) external view returns (string memory) {
        ...
        return string(abi.encodePacked(
            uint_to_string(tournament.timestamp), ",",
            uint_to_string(tournament.players[0]), ",", uint_to_string(tournament.players[1]), ",", uint_to_string(tournament.players[2]), ",", uint_to_string(tournament.players[3])
        ));
    }
    ...
    function test_sub(uint64 id) external view returns (string memory) {
        ...
        return string(abi.encodePacked(
            uint_to_string(tournament.sub_games[0].game_id), ",", uint_to_string(tournament.sub_games[0].player1), ",", 
            uint_to_string(tournament.sub_games[0].player2), ",", uint_to_string(tournament.sub_games[0].score1), ",",
